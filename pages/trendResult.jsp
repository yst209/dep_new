...
function tooltip_show(tooltipId, parentId)
{
    ...
    it.innerHTML = "<b><font size='2'>" + tootip + "<br/><br/>Project List:</font></b><br/>";
  	...
  	var monthlyBaselinePlus15Map = {
  		    <c:forEach items="${trendInfo.monthlyMap}" var="entry" varStatus="loop">
		        "${entry.key}": "<div style='float:left;margin-right:10px;margin-top:15px;'><c:forEach var="project" items="${entry.value.projectList}" varStatus="inside_loop">${project.projectId} - <fmt:formatNumber pattern="0.0" value="${project.baselineDuration*1.15}"/>  ${inside_loop.index== (fn:length(entry.value.projectList) < 25 ? 25 :  (fn:length(entry.value.projectList) % 2 == 0 ? fn:length(entry.value.projectList)/2-1 : (fn:length(entry.value.projectList)-1)/2)   )    ? '</div><div style=\'float:left;\'>' : ''} ${!inside_loop.last ? '<br/>' : ''}</c:forEach>${!loop.last ? '<br/></div>",' : '<br/></div>"'}
  		    </c:forEach>};


  	if(data_set=="4" && comparedMonthlyMap[20+date.substring(4) + monthNumber(date.substring(0,3))] != null)
  	...
  	else if(data_set=="1" && monthlyBaselinePlus15Map[20+date.substring(4) + monthNumber(date.substring(0,3))] != null)
  		it.innerHTML+=monthlyBaselinePlus15Map[20+date.substring(4) + monthNumber(date.substring(0,3))];
  	else if((data_set=="2" || data_set=="3") && monthlyMap[20+date.substring(4) + monthNumber(date.substring(0,3))] != null)
