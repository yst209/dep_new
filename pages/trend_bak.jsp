...
function tooltip_show(tooltipId, parentId)
{
    ...
    it.innerHTML = "<b><font size='2'>" + tootip + "<br/><br/>Project List:</font></b><br/>";
  	...
  	var monthlyMap = {
  		    <c:forEach items="${monthlyMap}" var="entry" varStatus="loop">
  		        "${entry.key}": "<c:forEach var="project" items="${entry.value.projectList}" varStatus="inside_loop">${project.projectId} - ${project.currentDuration} ${!inside_loop.last ? '<br/>' : '<br/>"'}</c:forEach>${!loop.last ? ',' : ''}
  		    </c:forEach>};
  	...
  	var monthlyBaselineMap = {
  		    <c:forEach items="${monthlyMap}" var="entry" varStatus="loop">
  		        "${entry.key}": "<c:forEach var="project" items="${entry.value.projectList}" varStatus="inside_loop">${project.projectId} - ${project.baselineDuration} ${!inside_loop.last ? '<br/>' : '<br/>"'}</c:forEach>${!loop.last ? ',' : ''}
  		    </c:forEach>};
	...
	it.style.top = parseInt(document.getElementById('cursorY').value) -5 +'';
...
<input type="hidden" id="cursorY"/>
	<div style="width:1200px;height:700px;text-align: center;">
	<div style="font-size:9pt; font-family:verdana;margin: auto;">
